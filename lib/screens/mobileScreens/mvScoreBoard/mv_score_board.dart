...
import '../../deskopView/sportsView/cricScore/score_widgets.dart';

// Root scoreboard entry that combines fetched data and live SignalR updates.
class MVScoreDashboardNew2 extends StatelessWidget {
  const MVScoreDashboardNew2({super.key, required this.eventId});
...
String _buildMatchMessageText(ScoreBoardModel scoreBoard, Inning currentInning) {
  ...
  if (target != null && target.runsToWin > 0 && target.ballsRemaining > 0) {
  ...
  if (scoreBoard.result.status.isNotEmpty) {
    return scoreBoard.result.status;
  }
