...
import '../../sportsView/fancyBetView/show_fancy_book_view.dart';
import '../../sportsView/matchOddsView/match_odds_header.dart';
import 'mm_fancy_bet_header.dart';
import 'mm_fancy_placebet_card.dart';

class MmFancyBetTile extends StatefulWidget {
  const MmFancyBetTile({
    super.key,
    required this.idx,
  ...
  final MMFancyMarketData fancyBet;
  final Function(int) action;

  @override
  State<MmFancyBetTile> createState() => _MmFancyBetTileState();
}

class _MmFancyBetTileState extends State<MmFancyBetTile> {
  ...
  void detectPriceChanges(MMFancyRunner? newRunner) {
            ...
            children: [
              Row(
                children: [
                  ...
                  Column(
                    children: [
                      BlocBuilder<SignalRMMFancyDataBloc, SignalRMMFancyDataState>(
                        builder: (_, frd) {
                          MMFancyRunner? runner;
                          final eventId = widget.fancyBet.eventId;

                          if (frd is SignalRMMFancyDataSuccess) {
                            final market = frd.fancy.values.where((m) => m.marketId == marketId && m.eventId == eventId).toList();
                  ...
                  MmFancyMmInfo(bet: bet),
                ],
              ),
              BlocListener<SignalRMMFancyDataBloc, SignalRMMFancyDataState>(
                listenWhen: (prev, curr) => curr is SignalRMMFancyDataSuccess,
                listener: (context, state) {
                  if (state is! SignalRMMFancyDataSuccess) return;

                  final marketId = widget.fancyBet.marketId;
                  final eventId = widget.fancyBet.eventId;
                  final marketList = state.fancy.values.where((m) => m.marketId == marketId && m.eventId == eventId).toList();
