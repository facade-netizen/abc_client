...
import 'score_board_iframe.dart';
import 'score_widgets.dart';

// Root scoreboard entry that combines fetched data and live SignalR updates.
class ScoreDashboardWithIframeLogic extends StatelessWidget {
  const ScoreDashboardWithIframeLogic({super.key, required this.eventId, required this.sportName});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FetchScoreBoardBloc, FetchScoreBoardState>(
      builder: (context, fetchState) {
        if (fetchState is FetchScoreBoardSuccess) {
          ScoreBoardModel? fetchData = fetchState.scoreBoard;
          return BlocBuilder<JoinMatchSignalRBloc, JoinMatchSignalRState>(
            builder: (context, signalRState) {
              ...
              ScoreBoardModel? currentData = signalRData ?? fetchData;
              if (int.parse(eventId) != currentData.matchId) {
              ...
              final String bowlingTeamName = currentInning.bowlingTeam.trim().isNotEmpty
                  ...
                  : orderedTeams.firstWhere((team) => team.toLowerCase() != battingTeamName.toLowerCase(), orElse: () => battingTeamName);
        ...
        } else if (fetchState is FetchScoreBoardFailure) {
          return ScoreBoardStreamer(eventId: eventId, sportName: sportName);
        } else {
...
class _ScoreDashboardNewCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final _PlayerLine striker = _buildBatterLine(batting: currentInning.batting, preferredName: scoreBoard.currentState.striker, fallbackIndex: 0);
    final _PlayerLine nonStriker = _buildBatterLine(batting: currentInning.batting, preferredName: scoreBoard.currentState.nonStriker, fallbackIndex: 1);
    final _PlayerLine bowler = _buildBowlerLine(bowling: currentInning.bowling, preferredName: scoreBoard.currentState.bowler);

    return Container(
      width: double.infinity,
      decoration: BoxDecoration(
        gradient: const LinearGradient(colors: [Color(0xFF10243A), Color(0xFF244867), Color(0xFF152434)], begin: Alignment.topLeft, end: Alignment.bottomRight),
        border: Border.all(color: const Color(0xFF78A6CC)),
        boxShadow: const [BoxShadow(color: Color(0x26162E45), blurRadius: 14, offset: Offset(0, 6))],
      ),
      child: Stack(
        ...
        children: [
          Positioned.fill(
            child: IgnorePointer(
              child: CustomPaint(
                painter: ScoreboardBackdropPainter(leftAccent: _teamAccent(battingTeamName), rightAccent: _teamAccent(bowlingTeamName)),
              ),
            ),
          ),
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _BallTimelineStrip(balls: _recentBalls(currentInning.lastBalls)),
                const SizedBox(height: 5),
                _ScoreFooter(striker: striker, nonStriker: nonStriker, bowler: bowler, scoreBoard: scoreBoard, currentInning: currentInning, previousInning: previousInning),
              ],
            ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        ...
        Expanded(child: _CurrentInningScore(currentInning: currentInning)),
        Expanded(
          child: Align(
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          textAlign: TextAlign.center,
          style: const TextStyle(color: Color(0xFFD9E9F7), fontSize: 15, fontWeight: FontWeight.w700, height: 1),
        ),
      ],
    );
  }
...
class _TeamIdentity extends StatelessWidget {
  const _TeamIdentity({required this.teamName, required this.logoUrl, required this.accent, required this.alignEnd, required this.isBattingNow, this.subtitle});
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        Column(
          ...
          children: [
                ...
                child: logoUrl.isEmpty
                    ? Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          gradient: LinearGradient(colors: [accent.withValues(alpha: 0.95), accent.withValues(alpha: 0.45)], begin: Alignment.topLeft, end: Alignment.bottomRight),
            ...
            Text(
              ...
              style: const TextStyle(color: Color(0xFFD6E8F6), fontSize: 15, fontWeight: FontWeight.w700),
            ),
            if (subtitle?.trim().isNotEmpty ?? false) ...[
              ...
              Text(
                subtitle!,
                textAlign: alignEnd ? TextAlign.right : TextAlign.left,
                style: const TextStyle(color: Color(0xFFB8D4F0), fontSize: 11, fontWeight: FontWeight.w600),
              ),
            ],
          ],
        ),
        ...
        if (isBattingNow) Positioned(top: 0, right: -20, child: LivePulseBadge(alignEnd: false, child: const _AnimatedBattingDot())),
      ],
    );
  }
}
...
class _BallTimelineStrip extends StatelessWidget {
  const _BallTimelineStrip({required this.balls});
      ...
      child: Row(
        ...
        children: [
          for (int gi = 0; gi < groups.length; gi++) ...[
            for (int bi = 0; bi < groups[gi].balls.length; bi++) ...[_buildBallChip(groups[gi].balls[bi].ball), if (bi < groups[gi].balls.length - 1) const SizedBox(width: 6)],
...
class _OverLabel extends StatelessWidget {
  const _OverLabel({required this.over});
...
class _ScoreFooter extends StatelessWidget {
  const _ScoreFooter({required this.striker, required this.nonStriker, required this.bowler, required this.scoreBoard, required this.currentInning, required this.previousInning});
        ...
        children: [
            ...
            child: Align(
              ...
              child: Column(
                children: [
                  ...
                  _TossMessage(tossWinner: scoreBoard.toss.tossWinner, tossDecision: scoreBoard.toss.tossDecision),
                ],
              ),
            ),
          ),
          Expanded(
            child: Column(
              ...
              children: [
                if (previousInning != null) _CompletedInningScore(teamName: previousInning!.battingTeam, inning: previousInning!),
                _FooterPlayer(name: bowler.name, stats: bowler.stats, alignEnd: true, iconType: _FooterIconType.ball),
              ],
            ),
          ),
        ],
...
class _MatchMessage extends StatelessWidget {
  const _MatchMessage({required this.scoreBoard, required this.currentInning});
...
class _TossMessage extends StatefulWidget {
  const _TossMessage({required this.tossWinner, required this.tossDecision});
...
class _CompletedInningScore extends StatelessWidget {
  const _CompletedInningScore({required this.teamName, required this.inning});
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(
            teamName,
            style: const TextStyle(color: Color(0xFFB8D4F0), fontSize: 9, fontWeight: FontWeight.w600, height: 1),
          ),
          const SizedBox(width: 4),
          Text(
            '${inning.score.runs}/${inning.score.wickets}',
            style: const TextStyle(color: Color(0xFFF3FAFF), fontSize: 11, fontWeight: FontWeight.w800, height: 1),
          ),
          const SizedBox(width: 4),
          Text(
            '(${inning.score.overs})',
            style: const TextStyle(color: Color(0xFFDBECFF), fontSize: 9, fontWeight: FontWeight.w600, height: 1),
          ),
        ],
      ),
    );
...
class _FooterPlayer extends StatelessWidget {
  const _FooterPlayer({required this.name, required this.stats, this.alignEnd = false, this.iconType = _FooterIconType.none});
    ...
    return name.isEmpty || name == '-'
        ...
        : Padding(
            ...
            child: Row(
              ...
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                if (!alignEnd && iconData != null) ...[Icon(iconData, size: 13, color: iconColor), const SizedBox(width: 6)],
                Flexible(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    ...
                    children: [
                      ...
                      if (stats.isNotEmpty) ...[
                        ...
                        Text(
                          stats,
                          textAlign: textAlign,
                          style: const TextStyle(color: Color(0xFFDBECFF), fontSize: 10.5, fontWeight: FontWeight.w800, height: 1),
                        ),
                      ],
                    ],
                  ),
...
_PlayerLine _buildBatterLine({required List<Batting> batting, required String? preferredName, required int fallbackIndex}) {
  ...
  if (normalizedPreferredName.isNotEmpty) {
    for (final Batting batter in batting) {
      if (batter.name.toLowerCase() == normalizedPreferredName.toLowerCase()) {
        return _PlayerLine(name: batter.name, stats: '${batter.runs} (${batter.balls})${batter.strikeRate > 0 ? '  SR ${batter.strikeRate.toStringAsFixed(1)}' : ''}');
      }
    }
  ...
  if (batting.isNotEmpty) {
    ...
    return _PlayerLine(name: batter.name, stats: '${batter.runs} (${batter.balls})');
  }
...
_PlayerLine _buildBowlerLine({required List<Bowling> bowling, required String? preferredName}) {
  final String normalizedPreferredName = preferredName?.trim() ?? '';

  if (normalizedPreferredName.isNotEmpty) {
    for (final Bowling bowler in bowling) {
      if (bowler.name.toLowerCase() == normalizedPreferredName.toLowerCase()) {
        return _PlayerLine(name: bowler.name, stats: '${bowler.runs}/${bowler.wickets} (${bowler.overs})');
      }
    }
    return _PlayerLine(name: normalizedPreferredName, stats: '');
  }
...
Widget _buildBallChip(String ball) {
  ...
  return Container(
    ...
    child: Text(
      style.text,
      style: TextStyle(color: style.foreground, fontSize: style.text.length > 1 ? 7.5 : 10.5, fontWeight: FontWeight.w800, height: 1),
    ),
  );
}
...
_BallStyle _getBallStyle(String ball) {
  switch (ball) {
    case 'W':
      return const _BallStyle(text: 'W', background: Color(0xFFBC2E2E), foreground: Colors.white);
    case '4':
      return const _BallStyle(text: '4', background: Color(0xFF1E8E3E), foreground: Colors.white);
    case '6':
      return const _BallStyle(text: '6', background: Color(0xFF1E8E3E), foreground: Colors.white);
    case 'NB':
      return const _BallStyle(text: 'NB', background: Color(0xFF996515), foreground: Colors.white);
    case 'WD':
      return const _BallStyle(text: 'WD', background: Color(0xFF2C6DB3), foreground: Colors.white);
    default:
      return _BallStyle(text: ball, background: const Color(0xFF23415E), foreground: const Color(0xFFF4FAFF));
  }
}
...
class _BallStyle {
  ...
  const _BallStyle({required this.text, required this.background, required this.foreground});
}
...
class _PlayerLine {
  ...
  const _PlayerLine({required this.name, required this.stats});
}
...
class _AnimatedBattingDotState extends State<_AnimatedBattingDot> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scaleAnimation = Tween<double>(begin: 0.9, end: 1.2).animate(CurvedAnimation(parent: _controller, curve: Curves.easeInOut));

    _opacityAnimation = Tween<double>(begin: 0.55, end: 1).animate(CurvedAnimation(parent: _controller, curve: Curves.easeInOut));
  }
      ...
      child: ScaleTransition(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: const [BoxShadow(color: Color(0x551E8E3E), blurRadius: 8, spreadRadius: 1)],
          ),
        ),
      ),
    );
