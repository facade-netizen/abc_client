...
import 'sports_bet_slip_order_card_for_odds.dart';

class BetSectionForOdds extends StatefulWidget {
  final bool isBack;
  final List<MapEntry<int, BetSlipItemForOdds>> bets;
  final double width;
  final void Function(int index)? onRemove;
  final void Function()? onStakeOrPriceChanged;

  const BetSectionForOdds({
    super.key,
  ...
  @override
  State<BetSectionForOdds> createState() => _BetSectionForOddsState();
}

class _BetSectionForOddsState extends State<BetSectionForOdds> {
  @override
  void initState() {
    super.initState();
    _setupControllers();
  ...
  @override
  void didUpdateWidget(covariant BetSectionForOdds oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, List<MapEntry<int, BetSlipItemForOdds>>> groupedBets = {};
          ...
          return Column(
            children: [
              ...
              ...eventBets.map((entry) {
                ...
                final BetSlipItemForOdds bet = entry.value;
                                                ...
                                                children: [
                                                  Expanded(
                                                    child: InkWell(
                                                      onTap: () {
                                                        setState(() {
                                                          bet.updatePrice(_incrementPrice(bet.price));
                                                        });
                                                        widget.onStakeOrPriceChanged?.call();
                                                      },
                                                      child: const Icon(
                                                  ...
                                                  Expanded(
                                                    child: InkWell(
                                                      onTap: () {
                                                        setState(() {
                                                          bet.updatePrice(_decrementPrice(bet.price));
                                                        });
                                                        widget.onStakeOrPriceChanged?.call();
                                                      },
                                                      child: const Icon(
