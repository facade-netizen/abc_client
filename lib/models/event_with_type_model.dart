class SportsResponse {
  ...
  SportsResponse({required this.status, required this.data, required this.message});

  factory SportsResponse.fromJson(Map<String, dynamic> json) {
    return SportsResponse(
      status: json['status'] ?? 0,
      data: (json['data'] as List<dynamic>?)?.map((e) => Sport.fromJson(e as Map<String, dynamic>)).toList() ?? [],
...
class Sport {
  ...
  Sport({required this.id, required this.name, required this.event});

  factory Sport.fromJson(Map<String, dynamic> json) {
    return Sport(id: json['id'] ?? 0, name: json['name'] ?? '', event: (json['event'] as List<dynamic>?)?.map((e) => Event.fromJson(e as Map<String, dynamic>)).toList() ?? []);
  }
}
