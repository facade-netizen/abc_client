...
class RulesAndRegulationsPopup {
  static Future<void> open(BuildContext context) async {
    if (kIsWeb) {
      _openWebPopup();
    } else {
      await Navigator.of(context).push(
        MaterialPageRoute(builder: (_) => const RulesAndRegulationsPage()),
      );
    }
  }
  ...
  static void _openWebPopup() {
    ...
    final popup = web.window.open('', 'rulesAndRegulations', options);
    if (popup != null) {
      final htmlContent = _buildRulesHtml();
      popup.document.open();
  ...
  static String _buildRulesHtml() {
    final htmlBody = _buildWebBodyHtml();
    return '''<!DOCTYPE html><html><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>6Ball Rules and Regulations</title>
<style>
...
.wrapper{max-width:980px;margin:24px auto;padding:0 20px;}
.card{background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 16px 40px rgba(15,23,42,.08);border:1px solid rgba(15,23,42,.08);}
.header{display:flex;align-items:center;padding:24px 24px 0;}
.mark{width:6px;height:36px;background:#FBBF24;border-radius:999px;margin-right:14px;box-shadow:0 0 0 4px rgba(251,191,36,.12);}
.header h1{margin:0;font-size:32px;font-weight:800;letter-spacing:-.02em;color:#111827;}
.content{padding:0 24px 28px;}
.note-box{margin:0 0 24px;padding:18px 20px;background:#E0F2FE;border:1px solid #93C5FD;border-radius:16px;color:#0F172A;}
.note-box strong{display:block;margin-bottom:10px;font-size:15px;}
.note-box p{margin:0 0 10px;font-size:15px;line-height:1.75;}
.text-block{font-size:15px;line-height:1.75;color:#374151;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica,Arial,sans-serif;}
.text-block h2{margin:28px 0 14px;font-size:22px;font-weight:800;color:#111827;}
.text-block h3{margin:24px 0 12px;font-size:18px;font-weight:700;color:#111827;}
.text-block p{margin:0 0 14px;}
.bullet, .sub-bullet{display:flex;align-items:flex-start;gap:10px;margin-bottom:10px;}
.bullet span:first-child, .sub-bullet span:first-child{font-size:15px;line-height:1.75;}
.sub-bullet{margin-left:18px;}
</style>
</head><body>
<div class="wrapper"><div class="card"><div class="header"><span class="mark"></span><h1>6Ball Rules and Regulations</h1></div><div class="content"><div class="text-block">
$htmlBody
</div></div></div></div>
</body></html>''';
  }

  static String _buildWebBodyHtml() {
    final body = _getBodyText();
    final lines = body.split('\n');
    final buffer = StringBuffer();
    buffer.writeln(
        '<div class="note-box"><strong>NOTE:</strong><p>Players using VPN and login from different IP frequently may result to void bets.</p><p>And on the basis of different IP from multiple city we can suspend the account and void bets.</p></div>');

    String? paragraph;
    void flush() {
      if (paragraph != null) {
        buffer.writeln('<p>${_escapeForHtml(paragraph!)}</p>');
        paragraph = null;
      }
    }
    ...
    for (final rawLine in lines) {
      ...
      if (_isSectionTitle(line)) {
        flush();
        buffer.writeln('<h2>${_escapeForHtml(line)}</h2>');
        continue;
      }
      if (_isSubsectionTitle(line)) {
        flush();
        buffer.writeln('<h3>${_escapeForHtml(line)}</h3>');
        continue;
      }
      if (_isBullet(line)) {
        flush();
        buffer.writeln('<div class="bullet"><span>•</span><span>${_escapeForHtml(line.substring(2).trim())}</span></div>');
        continue;
      }
      if (_isArrowBullet(line)) {
        flush();
        buffer.writeln('<div class="sub-bullet"><span>➢</span><span>${_escapeForHtml(line.substring(1).trim())}</span></div>');
        continue;
      }
      if (paragraph == null) {
        paragraph = line;
  ...
  static bool _isSectionTitle(String line) {
    return line == 'Exchange Rules and Regulations' || line.startsWith('Part ');
  }

  static bool _isSubsectionTitle(String line) {
    return line == 'Restricted territory information' || RegExp(r'^\d+\.\s').hasMatch(line) || RegExp(r'^[a-z]\)\s').hasMatch(line);
  }
  ...
  static bool _isArrowBullet(String line) {
    return line.startsWith('➢ ');
  }

  static String _getBodyText() {
    var body = rulesText;
    const titlePrefix = '6Ball Rules and Regulations\n';
    if (body.startsWith(titlePrefix)) {
      body = body.substring(titlePrefix.length);
    }
    const notePrefix =
    ...
    if (body.startsWith(notePrefix)) {
      body = body.substring(notePrefix.length);
    }
    return body.trimLeft();
  }

  static String _escapeForHtml(String value) {
    return value.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
  }

  static const String rulesText = '''6Ball Rules and Regulations
NOTE:
Players using VPN and login from different IP frequently may result to void bets.
And on the basis of different IP from multiple city we can suspend the account and void bets.
All 6Ball users including ‘Super’, ‘Master’ and ‘Sub’ account holders are advised to read the following ‘Terms and Conditions’. All users who use 6Ball agree and accept to the following:
1. The site www.6Ball.com and all of its original content are the sole property of ‘6Ball’ and are, as such, fully protected by International Copyright and other intellectual property rights laws.
2. Any form of ‘Passing of funds’, ‘Self Matching’ will not be tolerated on 6Ball. Any users found participating in such activites will be locked with the funds being reversed. Accounts participating in such activities must note that 6Ball reserves the right to Void any bets of such nature at any time within 1 week of the bet being placed.
...
4. Please note that if any account has been locked due to ‘any sort of cheating or suspicious’ within the last 72 hours, Company reserves the right to void any bet of this nature within the account irrespective of when the bet was placed.
5. No Arguments or claim in the above context will be entertained by 6Ball and the decision made by 6Ball will stand as final.
6. 6Ball Endeavors to have our services run 24 hours a day. However due to any technical issue, or disruption of services from our provider (Betfair.com), 6Ball will not be liable for any market positions that any account holders may hold.
7. Betfair.com reserves the right to resettle or void any market for reasons such as technical delayed suspension from the provider. In any such case 6Ball will settle according to Betfair.com
...
10. Any bets which are deemed of being suspicious, including bets which have been placed from the stadium or from a source at the stadium maybe voided at anytime. The decision of whether to void the particular bet in question or to void the entire market will remain at the discretion of 6Ball. The final decision of whether bets are suspicious will be taken by 6Ball and that decision will be full and final.
11. Any sort of cheating bet , any sort of Matching (Passing of funds), Court Siding (Ghaobaazi on commentary), Sharpening, Commission making is not allowed in 6Ball, If any 6Ball User is caught in any of such act then all the funds belonging that account would be seized and confiscated. No argument or claim in that context would be entertained and the decision made by 6Ball management will stand as final authority.
12. Fluke hunting/Seeking is prohibited in 6Ball, All the fluke bets will be reversed. Cricket commentary is just an additional feature and facility for 6Ball user but 6Ball is not responsible for any delay or mistake in commentary.
...
o	if the event has a scheduled 'off' time, all bets matched after that scheduled off time will be void; and
o	if the event does not have a scheduled 'off' time, it will use its reasonable endeavours to ascertain the time of the actual 'off' and all bets after the time of the 'off' determined will be void.
...
• For any team sport: if the scheduled venue is changed after the market has been loaded, all bets will be void only if the new venue is a home ground of the original away team
• For all categories or markets other than team sports: if the scheduled venue is changed after the market has been loaded, all bets will stand.
...
• In the event of a match being decided by a bowl-off or toss of the coin, all bets will be void except for those markets which have been unconditionally determined.
➢ Test matches
...
• With the exception of a player or team conceding a hole (where they are deemed a loser), if a hole is not completed by any player or team (other than for withdrawal or disqualification) all bets on that hole will be void unless the market is unconditionally determined.
• Any player or team withdrawing or being disqualified having played a stroke on that hole will be settled as a loser providing at least one other player or team completes that hole.
...
class RulesAndRegulationsPage extends StatelessWidget {
  ...
  static const _noteLabelStyle = TextStyle(
    fontSize: 15,
    fontWeight: FontWeight.w700,
    color: Color(0xFF0F172A),
  );

  static const _noteTextStyle = TextStyle(
    fontSize: 15,
    height: 1.75,
    color: Color(0xFF0F172A),
  );
  ...
  List<Widget> _buildContentWidgets() {
    ...
    for (final rawLine in lines) {
      ...
      if (_isSectionTitle(line)) {
        widgets.add(Text(line, style: _sectionTitleStyle));
        widgets.add(const SizedBox(height: 12));
        continue;
      }
      if (_isSubsectionTitle(line)) {
        widgets.add(Text(line, style: _subsectionTitleStyle));
      ...
      if (_isBullet(line)) {
        widgets.add(Padding(
          ...
          child: Row(
            ...
            children: [
              const Text('• ', style: _bulletStyle),
              Expanded(child: Text(line.substring(2).trim(), style: _bulletStyle)),
            ],
          ),
        ));
        continue;
      ...
      if (_isArrowBullet(line)) {
        widgets.add(Padding(
          ...
          child: Row(
            ...
            children: [
              const Text('➢ ', style: _bulletStyle),
              Expanded(child: Text(line.substring(1).trim(), style: _bulletStyle)),
            ],
          ),
        ));
        continue;
      }
      widgets.add(Text(line, style: _bodyStyle));
    }
  ...
  String _getBodyText() {
    var body = RulesAndRegulationsPopup.rulesText;
    const titlePrefix = '6Ball Rules and Regulations\n';
    if (body.startsWith(titlePrefix)) {
      body = body.substring(titlePrefix.length);
    }
  ...
  bool _isSubsectionTitle(String line) {
    return line == 'Restricted territory information' || RegExp(r'^\d+\.\s').hasMatch(line) || RegExp(r'^[a-z]\)\s').hasMatch(line);
  }
  ...
  bool _isArrowBullet(String line) {
    return line.startsWith('➢ ');
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('6Ball Rules and Regulations'),
        centerTitle: false,
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: const [
                        Text('NOTE:', style: _noteLabelStyle),
                        SizedBox(height: 10),
                        Text(
                          'Players using VPN and login from different IP frequently may result to void bets.',
